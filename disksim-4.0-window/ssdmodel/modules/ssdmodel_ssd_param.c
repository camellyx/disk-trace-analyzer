...
static int SSDMODEL_SSD_UNHEALTHY_PAGES_PERCENTAGE_depend(char *bv) {
return -1;
}

static void SSDMODEL_SSD_UNHEALTHY_PAGES_PERCENTAGE_loader(struct ssd * result, int i) { 
if (! ((i >= 0))) { // foo 
 } 
 result->params.unhealthy_blocks = i;
...
(void *)SSDMODEL_SSD_UNHEALTHY_PAGES_PERCENTAGE_loader,
...
SSDMODEL_SSD_UNHEALTHY_PAGES_PERCENTAGE_depend,
