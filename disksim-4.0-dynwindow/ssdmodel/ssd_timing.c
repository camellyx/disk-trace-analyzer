...
void _ssd_alloc_active_healthy_block(int plane_num, int elem_num, ssd_t *s) // healthy
{
    ...
    if (active_block != -1) {
        ...
        metadata->plane_meta[plane_num].window[metadata->plane_meta[plane_num].window_ptr++] = active_block;
        metadata->plane_meta[plane_num].window_ptr %= s->params.window_size;
...
static double ssd_write_one_active_page(int blkno, int count, int elem_num, ssd_t *s)
{
    ...
    if (ssd_last_page_in_block(metadata->active_healthy_page, s)) {
        ...
        // do we need to create more free blocks for future writes?
        if (ssd_start_cleaning_healthy(-1, elem_num, s)) {
